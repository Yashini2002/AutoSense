...
import { Activity, User, Mail, Lock, Eye, EyeOff, Phone, ArrowRight, Check } from 'lucide-react';
...
export default function SignUpScreen() {
  ...
  const handleSignUp = (e: React.FormEvent) => {
    ...
    alert('Sign up successful - Navigate to OTP Verification');
  };
                  ...
                  {passwordRequirements.map((req, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      <div className={`w-4 h-4 rounded-full flex items-center justify-center ${req.met ? 'bg-green-500' : 'bg-slate-300'}`}>
                        {req.met && <Check className="w-3 h-3 text-white" strokeWidth={3} />}
                      </div>
